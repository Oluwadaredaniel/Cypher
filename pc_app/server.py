...
MACROS_FILE = Path("pc_app/macros.json")
SETTINGS_FILE = Path("pc_app/settings.json")
...
def upload_file_stream():
    ...
    if not dest_path.exists():
        try:
            ...
            return jsonify({"success": False, "error": f"Could not create destination: {str(e)}"}), 500
    ...
    if path and os.path.exists(path):
        try:
            if os.path.isfile(path):
                ...
                shutil.rmtree(path) # Deletes folder and all contents
...
def get_screenshot():
    try:
        ...
        return jsonify({"success": True, "image": base64.b64encode(buffered.getvalue()).decode()})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
